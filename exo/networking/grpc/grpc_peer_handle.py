...
class GRPCPeerHandle(PeerHandle):
    def __init__(self, _id: str, address: str, device_capabilities: DeviceCapabilities, is_wan: bool):
        self._id = _id
        self.address = address
        self._device_capabilities = device_capabilities
        self.is_wan = is_wan  # New attribute to store WAN status
        self.channel = None
        self.stub = None
    ...
    async def health_check(self) -> bool:
        try:
            ...
            if DEBUG >= 4:
                print(f"Health check failed for {self._id}@{self.address}. Error: {e}")
                import traceback
                traceback.print_exc()
            return False
